 ...
 * extend the functionality above that use by the VLA functions.
 */
...
struct vfloat4
{
	 ...
	 *
	 * Consider using vfloat4::loada() for 16-byte aligned data.
	 */
	ASTCENC_SIMD_INLINE explicit vfloat4(const float *a)
	{
		m = _mm_loadu_ps(a);
	}
	...
	/**
	 * @brief Construct from an existing 4-wide SIMD register.
	 */
	...
	/**
	 * @brief Set the scalar of a single lane.
	 */
	template <int l> ASTCENC_SIMD_INLINE void set_lane(float a)
	{
#if ASTCENC_SSE >= 41
		__m128 v = _mm_set_ps1(a);
	...
	/**
	 * @brief Color vector alias for lane<0>();
	 */
	...
	/**
	 * @brief Color vector alias for lane<1>();
	 */
	...
	/**
	 * @brief Color vector alias for lane<2>();
	 */
	...
	/**
	 * @brief Color vector alias for lane<3>();
	 */
	...
	/**
	 * @brief Position vector alias for lane<0>();
	 */
	ASTCENC_SIMD_INLINE float x() const
	{
		return lane<0>();
	}

	/**
	 * @brief Position vector alias for lane<1>();
	 */
	ASTCENC_SIMD_INLINE float y() const
	{
		return lane<1>();
	}

	/**
	 * @brief Position vector alias for lane<2>();
	 */
	ASTCENC_SIMD_INLINE float z() const
	{
		return lane<2>();
	}

	/**
	 * @brief Position vector alias for lane<3>();
	 */
	ASTCENC_SIMD_INLINE float w() const
	{
		return lane<3>();
	}
	 ...
	 */
	static ASTCENC_SIMD_INLINE vfloat4 load1(const float* a)
	{
		return vfloat4(_mm_load_ps1(a));
	}

	/**
	 * @brief Factory that returns a vector loaded from 16B aligned memory.
	 */
	static ASTCENC_SIMD_INLINE vfloat4 loada(const float* a)
	{
		return vfloat4(_mm_load_ps(a));
	}
...
struct vint4
{
	 ...
	 * @brief Construct from 4 values loaded from an unaligned address.
	 *
	 * Consider using vint4::loada() for 16-byte aligned data.
	 */
	ASTCENC_SIMD_INLINE explicit vint4(const int *a)
	{
		m = _mm_load_si128((const __m128i*)a);
	}
	 ...
	 * @brief Color vector alias for lane<0>();
	 */
	ASTCENC_SIMD_INLINE int r() const
	{
		return lane<0>();
	}
	 ...
	 * @brief Color vector alias for lane<1>();
	 */
	ASTCENC_SIMD_INLINE int g() const
	{
		return lane<1>();
	}
	 ...
	 * @brief Color vector alias for lane<2>();
	 */
	ASTCENC_SIMD_INLINE int b() const
	{
		return lane<2>();
	}
	 ...
	 * @brief Color vector alias for lane<3>();
	 */
	ASTCENC_SIMD_INLINE int a() const
	{
		return lane<3>();
	}
	 ...
	 * @brief Position vector alias for lane<0>();
	 */
	ASTCENC_SIMD_INLINE int x() const
	{
		return lane<0>();
	}
	 ...
	 * @brief Position vector alias for lane<1>();
	 */
	ASTCENC_SIMD_INLINE int y() const
	{
		return lane<1>();
	}
	 ...
	 * @brief Position vector alias for lane<2>();
	 */
	ASTCENC_SIMD_INLINE int z() const
	{
		return lane<2>();
	}
	 ...
	 * @brief Position vector alias for lane<3>();
	 */
	ASTCENC_SIMD_INLINE int w() const
	{
		return lane<3>();
	}
 ...
 *
 * bit0 = lane 0  .. bit3 = lane 3
 */
ASTCENC_SIMD_INLINE unsigned int mask(vmask4 v)
{
 ...
 */
ASTCENC_SIMD_INLINE bool any(vmask4 v)
{
	return mask(v) != 0;
}
 ...
 */
ASTCENC_SIMD_INLINE bool all(vmask4 v)
{
	return mask(v) == 0xF;
}
...
{
	v = min(v, vint4(_mm_shuffle_epi32(v.m, _MM_SHUFFLE(0, 0, 3, 2))));
	v = min(v, vint4(_mm_shuffle_epi32(v.m, _MM_SHUFFLE(0, 0, 0, 1))));
	return vint4(_mm_shuffle_epi32(v.m, _MM_SHUFFLE(0, 0, 0, 0)));
}
...
/**
 * @brief Pack low 8 bits of each lane into bottom 32 bits of vector.
 */
...
/**
 * @brief Select @c b if @c cond, else @c a.
 */
...
{
	return vfloat4(_mm_mul_ps(a.m, _mm_set_ps1(b)));
}
...
{
	return vfloat4(_mm_mul_ps(_mm_set_ps1(a), b.m));
}
 ...
 *
 * It is assume that neither @c min nor @c max are NaN values. If @c a is NaN
 * then @c min will be returned for that lane.
 */
ASTCENC_SIMD_INLINE vfloat4 clamp(float min, float max, vfloat4 a)
{
	// Do not reorder - second operand will return if either is NaN
	a.m = _mm_max_ps(a.m, _mm_set_ps1(min));
	a.m = _mm_min_ps(a.m, _mm_set_ps1(max));
	return a;
}
...
{
	a.m = _mm_max_ps(a.m, _mm_setzero_ps());
	a.m = _mm_min_ps(a.m, _mm_set_ps1(1.0f));
	return a;
}
...
ASTCENC_SIMD_INLINE vfloat4 round(vfloat4 a)
{
	...
	__m128 no_fraction = _mm_set_ps1(8388608.0f);
...
/**
 * @brief Return the dot product of a vector.
 */
ASTCENC_SIMD_INLINE float dot(vfloat4 a, vfloat4 b)
{
...
/**
 * @brief Return a normalized (unit length) vector.
 */
ASTCENC_SIMD_INLINE vfloat4 normalize(vfloat4 a)
{
#if (ASTCENC_SSE >= 41) && (ASTCENC_ISA_INVARIANCE == 0)
	__m128 scale = _mm_rsqrt_ps(_mm_dp_ps(a.m, a.m, 0xFF));
	return vfloat4(_mm_mul_ps(a.m, scale));
#else
	float scale = astc::rsqrt(dot(a, a));
	return a * scale;
#endif
}
...
#else
	return vfloat4(std::sqrt(a.r()), std::sqrt(a.g()),
	               std::sqrt(a.b()), std::sqrt(a.a()));
#endif
}
